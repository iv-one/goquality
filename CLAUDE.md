...
- **`internal/check`**:
  ...
  - **Suppression:** each check must call `env.filter` or `env.suppressed` on its findings before computing its summary and score. The runner does not filter. Directives are parsed in `suppress.go`, and `nolintNames` maps check names to golangci-lint aliases.
